...
export class BluvoPreviewManager {
    ...
    private listeners: Set<(states: Record<string, WalletPreviewState>) => void> = new Set();
    ...
    async loadPreviewWallet(
        ...
        try {
            ...
            const balanceResponse = await this.options.fetchWithdrawableBalanceFn(walletId);
     ...
     */
    subscribe(listener: (states: Record<string, WalletPreviewState>) => void): () => void {
    ...
    private notifyListeners(): void {
        ...
        this.listeners.forEach(listener => listener(allStates));
    }
}
